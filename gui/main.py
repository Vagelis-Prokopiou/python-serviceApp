...
class RootWidget(BoxLayout):
    ...
    def kms_provided(self, *args):
        # If the provided value is all numbers.
        if regex_validate_num(args[0]):
            ...
            self.ids.results_label.text = RootWidget.error_messages[args[1]]

    def check(self):
        ''' Run this when the check button is pressed. '''
        if RootWidget.proceed:
            # global spare_parts_list
            ...
            for x in range(1, len(RootWidget.spare_parts_list)):
                ...
                if compare_dates(date_changed, RootWidget.today, date_interval):
                    RootWidget.messages.append(
                        ...
                        'You must change the {1} again now!'.format(
                            (RootWidget.spare_parts_list[x][2].lower()), RootWidget.spare_parts_list[x][0].lower()))
                ...
                if compare_kms(RootWidget.global_kms, kms_changed, kms_interval):
                    RootWidget.messages.append(
                        ...
                        'You must change the {1} again now!'.format(
                            (RootWidget.spare_parts_list[x][4]).lower(), RootWidget.spare_parts_list[x][0].lower()))
